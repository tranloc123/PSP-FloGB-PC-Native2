...
if len(sys.argv)!=2: raise SystemExit('Usage: build_patch_chain_final.py <ppsspp_repo>')
ROOT=Path(__file__).resolve().parent; PROJECT=ROOT.parent; REPO=Path(sys.argv[1]).resolve(); PYTHON=sys.executable
def run(*args):
    cmd=[str(x) for x in args]; print('>>>',' '.join(cmd),flush=True); subprocess.run(cmd,check=True)
def patch_package_json():
    p=PROJECT/'package.json'; pkg=json.loads(p.read_text(encoding='utf-8'))
    ...
    p.write_text(json.dumps(pkg,ensure_ascii=False,indent=2)+'\n',encoding='utf-8'); print('FINAL package.json auto-patch: PASS')
print('=== PSP FloGB FINAL PATCH CHAIN ===')
...
fix10=ROOT/'tools'/'apply_pc_fix10_control_center.py'
if fix10.exists(): run(PYTHON,fix10,REPO)
else: print('NOTE: FIX10 not present')
run(PYTHON,ROOT/'tools'/'apply_pc_fix11_rank_menu_toggle.py',REPO)
run(PYTHON,ROOT/'tools'/'apply_pc_fix12_p2_codemand_input.py',REPO)
patch_package_json()
print('=== FINAL PATCH CHAIN PASS THROUGH FIX12 ===')
