...
def patch_package_json():
    ...
    w = b.setdefault('win', {})
    w['target'] = ['nsis', 'portable']
    w.pop('artifactName', None)
    b.pop('artifactName', None)
    n = b.setdefault('nsis', {})
    n.setdefault('oneClick', False)
    n.setdefault('allowToChangeInstallationDirectory', True)
    n.setdefault('createDesktopShortcut', True)
    n.setdefault('shortcutName', 'PSP FloGB')
    n['artifactName'] = 'PSP-FloGB-Native-Unified-${version}-${arch}-Setup.${ext}'
...
if fix10.exists():
    run(PYTHON, fix10, REPO)
else:
    print('NOTE: FIX10 not present; continuing without FIX10')
run(PYTHON, ROOT / 'tools' / 'apply_pc_fix11_rank_menu_toggle.py', REPO)
patch_package_json()
print('=== FINAL PATCH CHAIN PASS THROUGH FIX11 ===')
